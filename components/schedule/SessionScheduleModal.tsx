...
export function SessionScheduleModal({
  ...
  const handleSave = async () => {
    try {
      ...
      if (existingSchedule) {
        ...
        response = await fetch(`/api/session-schedules/${existingSchedule.schedule_id}`, {
          method: "PATCH",
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={existingSchedule ? "Edit Session Schedule" : "Create Session Schedule"}>
        {/* Content */}
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <label htmlFor="is-active" className="text-sm font-medium text-gray-700">
              Active (sessions will be scheduled)
            </label>
          </div>
        </div>
